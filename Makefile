...
# Optional build time features; See min_mon.s
#
...
# Included by default and always passed; min_mon.s and custom_commands.s both
# fall back to 1 if it is missing, so a manual run of ca65 without it still gets
# the same ROM a plain "make" does
LCD ?= 1
...
# only matters when the assembler is in, but it is always passed so that, again,
# manual run of ca65 gets the same ROM a plain "make" does
ASM ?= 1
